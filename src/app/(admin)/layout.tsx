  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 right-4 z-[100] w-9 h-9 rounded-full bg-[#FF6B35] text-white flex items-center justify-center shadow-lg hover:scale-105 transition-transform"
        title="切换界面"
      >
        <Shield size={16} />
      </button>
      ...
      {showPortal && (
        <div className="fixed top-14 right-4 z-[100] bg-[#1E1E3A] rounded-xl shadow-2xl border border-[#2D2D4A] p-2 w-40 animate-float-up">
          ...
          <Link
            href="/admin"
            onClick={() => setShowPortal(false)}
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-[#FF6B35] bg-[#FF6B35]/10 font-medium"
          >
            <Shield size={14} /> 管理后台
          </Link>
        </div>
