    ...
    <div className="min-h-screen bg-[#2A2030]">
      ...
      {showPortal && (
        <div className="fixed top-14 right-4 z-[100] bg-[#362A3E] rounded-2xl shadow-2xl border border-[#4A3E52] p-2 w-44 animate-float-up">
          ...
          <Link href="/ai" onClick={() => setShowPortal(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-[#E8A0B5] bg-[#D4859B]/10 font-medium">
            <Brain size={14} /> AI 中台
          </Link>
          <Link href="/admin" onClick={() => setShowPortal(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-[#B8A8A4] hover:text-white hover:bg-[#4A3E52]">
            <Shield size={14} /> 管理后台
          </Link>
        </div>
      )}
